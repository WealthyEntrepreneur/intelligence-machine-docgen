...
// ============================================================
// Primary brand. NAVY/TEAL are `let` so a per-client brand kit can override them at
// generation time via setBrand(); resetBrand() restores the WE defaults.
let NAVY = "0C1C3B";   // Primary. Headlines, table headers, H1, buttons
let TEAL = "4393A5";   // Accent. H3, subheadings, highlight text, key insights
const NAVY_DEFAULT = "0C1C3B";
const TEAL_DEFAULT = "4393A5";
function setBrand(b) {
  const clean = (v) => String(v || "").replace(/[^0-9a-fA-F]/g, "").slice(0, 6);
  if (b && clean(b.primary)) NAVY = clean(b.primary).toUpperCase();
  if (b && clean(b.accent)) TEAL = clean(b.accent).toUpperCase();
}
function resetBrand() { NAVY = NAVY_DEFAULT; TEAL = TEAL_DEFAULT; }

// Title/H1/H2 font override (R-106: Momentum Profile reports render Title/H1/H2 in
// Georgia Bold; every other deliverable keeps Arial). H3 and body stay Arial regardless
// (set directly on their own TextRuns, never via TITLE_FONT). Mirrors the setBrand/
// resetBrand override pattern above; callers reset it after rendering.
let TITLE_FONT = "Arial";
const TITLE_FONT_DEFAULT = "Arial";
function setTitleFont(font) { TITLE_FONT = font || TITLE_FONT_DEFAULT; }
function resetTitleFont() { TITLE_FONT = TITLE_FONT_DEFAULT; }
...
function run(text, o = {}) {
  return new TextRun({ text, font: o.font || FONT, size: o.size || 22, color: o.color || BODY,
...
// H1 - Navy heading with bottom border. Font is TITLE_FONT (Arial by default, Georgia
// for Momentum Profile reports via setTitleFont - R-106).
function h1(text) {
  return new Paragraph({
    ...
    children: [new TextRun({ text, font: TITLE_FONT, size: 32, bold: true, color: NAVY })]
  });
}

// H2 - Navy subheading. Font is TITLE_FONT (see h1).
function h2(text) {
  return new Paragraph({
    heading: HeadingLevel.HEADING_2,
    spacing: { before: 280, after: 160 },
    children: [new TextRun({ text, font: TITLE_FONT, size: 26, bold: true, color: NAVY })]
  });
}
...
// Read type + pixel dimensions straight from an image buffer's header, so we can embed a
// client logo at its true aspect ratio without pulling in an image-size dependency.
// Covers the raster formats docx can embed (png/jpg/gif/bmp); returns null otherwise.
function sniffImage(buf) {
  ...
  if (buf[0] === 0x42 && buf[1] === 0x4d) {
    return { type: "bmp", width: buf.readInt32LE(18), height: buf.readInt32LE(22) };
  }
  if (buf[0] === 0xff && buf[1] === 0xd8) {
    ...
    while (off + 9 < buf.length) {
      ...
      // SOF0-SOF15 carry the frame dimensions; skip the non-SOF markers in that range.
...
// Cover logo paragraph: embed the client's logo image scaled to fit the cover slot at its
// true aspect ratio; fall back to the "[ Client logo ]" placeholder if none/unreadable.
function clientLogoParagraph(logoBuf, placeholderText) {
  const meta = sniffImage(logoBuf);
  if (meta && meta.width > 0 && meta.height > 0) {
    const MAXW = 210, MAXH = 96;
    const scale = Math.min(MAXW / meta.width, MAXH / meta.height, 1);
    const width = Math.max(1, Math.round(meta.width * scale));
    const height = Math.max(1, Math.round(meta.height * scale));
    return new Paragraph({ spacing: { before: 300, after: 0 }, alignment: AlignmentType.CENTER,
      children: [new ImageRun({ type: meta.type, data: logoBuf, transformation: { width, height } })] });
  }
  return new Paragraph({ spacing: { before: 300, after: 0 }, alignment: AlignmentType.CENTER,
    children: [run(placeholderText || "[ Client logo ]", { size: 20, italics: true, color: SLATE })] });
}

// Client-branded cover (Formatting Guidelines 1.7). Returns an array of paragraphs
// ending in a page break; pass as the `cover` option to buildDocument. `o.logo` is an
// optional image Buffer (the client's brand-kit logo) rendered at the top of the cover.
function coverClientBranded(o) {
  const c = [];
  c.push(clientLogoParagraph(o.logo, o.clientLogoText));
  c.push(new Paragraph({ spacing: { before: 4200, after: 120 }, alignment: AlignmentType.CENTER,
    children: [run(o.title, { size: 52, bold: true, color: NAVY, font: TITLE_FONT })] }));
...
// ============================================================
module.exports = {
  // Per-client brand override
  setBrand, resetBrand,
  // Title/H1/H2 font override (R-106)
  setTitleFont, resetTitleFont,
  // Brand colors
