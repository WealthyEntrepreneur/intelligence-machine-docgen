...
function renderDocx(job, brandKit, logoBuf) {
  // R-106: Momentum Profile reports always render in WE branding (Navy #0C1C3B / Teal
  // #4393A5, no gold), never the client's palette. All other deliverables use the client's
  // brand tokens (never WE Navy/Teal for client work).
  const isMomentum = job.doc_class === 'momentum_profile';
  if (!isMomentum && brandKit && (brandKit.primary_color || brandKit.accent_color)) {
    H.setBrand({ primary: brandKit.primary_color, accent: brandKit.accent_color });
  } else {
    H.resetBrand();
  }
  // R-106: Title/H1/H2 render in Georgia Bold for Momentum Profile reports only; every
  // other deliverable keeps Arial (H3 and body are unaffected either way).
  if (isMomentum) { H.setTitleFont('Georgia'); } else { H.resetTitleFont(); }
