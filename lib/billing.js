...
// POST /billing/overage-checkout { purchase_id }  — R-85 one-click allowance top-up.
// The purchase row was created by overage_purchase_create(), which ran the R-84
// membership gate and priced from the rate registry server-side. This endpoint
// never trusts an amount from the client — it charges what the DB row says.
async function stripeOverageCheckout(req, res) {
  const stripe = getStripe();
  if (!stripe) return res.status(503).json({ error: 'stripe_not_configured' });
  const { purchase_id, success_url, cancel_url } = req.body || {};
  if (!purchase_id) return res.status(400).json({ error: 'purchase_id required' });

  const { data: purchase } = await supabase.from('overage_purchases').select('*').eq('id', purchase_id).single();
  if (!purchase) return res.status(400).json({ error: 'unknown_purchase' });
  if (purchase.status !== 'pending') return res.status(400).json({ error: 'purchase_not_pending' });

  const auth = await authTenant(req, purchase.tenant_id);
  if (!auth.ok) return res.status(auth.code).json({ error: auth.error });

  const label = purchase.allowance_type.replace(/_/g, ' ');
  try {
    const sess = await stripe.checkout.sessions.create({
      mode: 'payment',
      line_items: [{
        price_data: {
          ...
          product_data: { name: `${purchase.qty} ${label} — allowance top-up` },
        },
        quantity: 1,
      }],
      metadata: { overage_purchase_id: purchase.id, tenant_id: purchase.tenant_id },
      success_url: success_url || 'https://www.intelligencemachine.ai/billing?overage=success',
      cancel_url: cancel_url || 'https://www.intelligencemachine.ai/billing?overage=cancel',
    });
    return res.json({ url: sess.url });
  } catch (e) {
    console.error('[billing] overage checkout failed', e.message);
    return res.status(500).json({ error: String(e.message || e) });
  }
}

// POST /billing/overage-invoice { run_id }  — R-68 automated overage billing.
// Called server-to-server by Postgres (bill_monthly_overages(), via pg_net), never by the
// browser. The run row was created idempotently by that function; this handler only acts on
// it if it's still 'pending', so a retried/duplicate call is a safe no-op. Creates a Stripe
// Invoice Item (rolls into the customer's next regular invoice — no separate immediate charge,
// no manual "buy" click from the client, which is exactly what distinguishes R-68 from R-85).
async function stripeOverageInvoice(req, res) {
  const { run_id } = req.body || {};
  if (!run_id) return res.status(400).json({ error: 'run_id required' });

  const { data: run } = await supabase.from('overage_billing_runs').select('*').eq('id', run_id).eq('status', 'pending').maybeSingle();
  if (!run) return res.json({ ok: true, skipped: 'not_pending_or_unknown' });

  const { data: client } = await supabase.from('clients').select('stripe_customer_id').eq('tenant_id', run.tenant_id).maybeSingle();
  if (!client || !client.stripe_customer_id) {
    await supabase.rpc('mark_overage_billing_run', { p_run_id: run_id, p_status: 'skipped_no_customer' });
    return res.json({ ok: true, skipped: 'no_stripe_customer' });
  }

  const stripe = getStripe();
  if (!stripe) {
    await supabase.rpc('mark_overage_billing_run', { p_run_id: run_id, p_status: 'failed', p_error: 'stripe_not_configured' });
    return res.status(503).json({ error: 'stripe_not_configured' });
  }

  const label = run.allowance_type.replace(/_/g, ' ');
  try {
    const item = await stripe.invoiceItems.create({
      customer: client.stripe_customer_id,
      amount: run.amount_cents,
      currency: 'usd',
      description: `${run.units} ${label} overage — automatic monthly billing`,
    });
    await supabase.rpc('mark_overage_billing_run', { p_run_id: run_id, p_status: 'invoiced', p_stripe_invoice_item_id: item.id });
    return res.json({ ok: true, invoiced: true, invoice_item_id: item.id });
  } catch (e) {
    console.error('[billing] overage invoice failed', e.message);
    await supabase.rpc('mark_overage_billing_run', { p_run_id: run_id, p_status: 'failed', p_error: String(e.message || e) });
    return res.status(500).json({ error: String(e.message || e) });
  }
}
  ...
  try {
    ...
    switch (event.type) {
      case 'checkout.session.completed':
        ...
        } else if (o.metadata && o.metadata.overage_purchase_id) {
          // R-85 allowance top-up (idempotent by session id inside the RPC).
          await supabase.rpc('overage_purchase_complete', { p_purchase_id: o.metadata.overage_purchase_id, p_stripe_session_id: o.id });
        }
        break;
...
module.exports = { stripeCheckout, stripeSubscribe, stripeOverageCheckout, stripeOverageInvoice, stripeWebhook };
