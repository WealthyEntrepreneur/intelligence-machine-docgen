...
async function stripeCheckout(req, res) {
  ...
  if (!jwt) { console.error('[billing] checkout: no bearer token reached the service'); return res.status(401).json({ error: 'no_token' }); }
  let user;
  try {
    const { data, error } = await supabaseAuth.auth.getUser(jwt);
    if (error) { console.error('[billing] getUser error:', error.message, '(token length', jwt.length + ')'); return res.status(401).json({ error: 'unauthorized' }); }
    user = data && data.user;
  } catch (e) {
    console.error('[billing] getUser threw:', e.message);
    return res.status(401).json({ error: 'unauthorized' });
  }
  if (!user) { console.error('[billing] getUser returned no user'); return res.status(401).json({ error: 'unauthorized' }); }
  ...
  const { data: u, error: uErr } = await supabase.from('users').select('tenant_id, role').eq('id', user.id).single();
  if (uErr) console.error('[billing] users lookup error:', uErr.message);
  if (!u || (u.tenant_id !== tenant_id && u.role !== 'we_admin')) {
    console.error('[billing] forbidden:', JSON.stringify({ userId: user.id, dbTenant: u && u.tenant_id, sentTenant: tenant_id, role: u && u.role }));
